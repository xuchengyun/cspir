...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    private void check() {
        ...
        if (!isBalanced()) System.out.println("不是黑平衡");
    ...
    //上界和下界
    public boolean isBST(Node node, Key min, Key max) {
    ...
    private void print() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (cur.left != null) {
                    System.out.println(" 的左孩子: " + cur.left.key + " 的颜色" + cur.left.color);
                    queue.offer(cur.left);
                }
                if (cur.right != null) {
                    System.out.println(" 的右孩子: " + cur.right.key + " 的颜色" + cur.right.color);
